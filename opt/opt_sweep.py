...
def log_image(image_name, image, global_steps, summary_writer=None):
    if summary_writer is not None:
        summary_writer.add_image(image_name, image, global_step=global_steps, dataformats='HWC')

    if WANDB_ON:
        wandb.log({image_name: wandb.Image(image.numpy())}, step=global_steps)
...
def main():
    ...
    config_util.define_common_args(parser, data_dir_optional=True)
    ...
    if WANDB_ON:
        wandb.init(project="roomoxels", anonymous="allow", config=vars(args))
        args = wandb.config
    ...
    if WANDB_ON:
        ...
        with torch.no_grad():
            ...
            print(f"rendering {n_images} frames")
            ...
            wandb.log({
                ...
                'final/video': wandb.Video(vid, fps=30, format="mp4")
            })


if WANDB_ON:
    import wandb
    wandb.agent(SWEEP_ID, function=main, count=30, entity="mackopes", project="roomoxels")