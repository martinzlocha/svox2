...
def load_image(fpath, scale=1):
    ...
    return torch.from_numpy(im_gt, dtype=torch.uint8)
...
def load_depth_file(fpath, width, height) -> torch.Tensor:
    ...
    return torch.from_numpy(depth, dtype=torch.float16)
...
def load_confidence_file(fpath, width, height) -> torch.Tensor:
    ...
    return torch.from_numpy(confidence_img, dtype=torch.uint8)
